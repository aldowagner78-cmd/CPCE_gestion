...
import { MedicalDocumentReview, type ReviewCompletedData } from '@/components/ai/MedicalDocumentReview';
import { classifyExpedientType, generateCoseguroNumber } from '@/services/classificationService';
import {
    ...
    Sparkles, ChevronRight, ChevronLeft, ClipboardCheck,
} from 'lucide-react';
...
const db = <T>(table: string): PostgrestQueryBuilder<any, T, T[], any> => supabase.from(table);
    ...
    fullPractice?: Practice; // To store the full practice object for re-request
}
    ...
    const [wizardStep, setWizardStep] = useState<WizardStep>(1);
        ...
        const [reviewData, setReviewData] = useState<ReviewCompletedData | null>(null);
    ...
    // ── Handler: Wizard Step 1 → Step 2 ──
    // Cuando el usuario completa la revisión del documento procesado por IA
    const handleReviewComplete = useCallback((data: ReviewCompletedData) => {
                        setReviewData(data);

                    // Generate coseguro number
                    const cosNum = generateCoseguroNumber();
                    setCoseguroNumber(cosNum);
                    setPrescriptionNumber(cosNum);

                    const aiData = data.rawAIData;

                    // Map validated fields to form fields
                    for (const field of data.fields) {
            if (!field.accepted || !field.value) continue;
                    switch (field.key) {
                case 'affiliateName':
                    case 'affiliate':
                    setAffSearch(field.value);
                    break;
                    case 'doctor':
                    setDoctorName(field.value);
                    break;
                    case 'doctorRegistration':
                    setDoctorRegistration(field.value);
                    break;
                    case 'prescriptionDate':
                    setPrescriptionDate(field.value);
                    break;
                    case 'diagnosisText': {
                    const diagCIE = data.fields.find(f => f.key === 'diagnosisCIE')?.value || '';
                    const diagTerms = aiData.diagnosisSearchTerms || [];
                    const bestSearch = diagCIE || diagTerms[0] || field.value.substring(0, 40);
                    ...
                    setAiDocumentFile(data.file);

                    // Build AI note
                    const noteLines: string[] = ['[IA] — Datos aceptados del pedido médico:'];
                    for (const field of data.fields) {
            if (field.accepted && field.value) {
                        noteLines.push(`• ${field.label}: ${field.value.substring(0, 100)}`);
            }
        }
        ...
        // Auto-classify expedient type based on practices text
        const practicesField = data.fields.find(f => f.key === 'practices');
                    if (practicesField?.value) {
            const dummyPractices = practicesField.value.split('\n').map(line => ({
                        id: 0, code: '', description: line.trim(), jurisdiction_id: 0,
                    financial_value: 0, category: '', created_at: '',
            }));
            const diagField = data.fields.find(f => f.key === 'diagnosisText');
                    const classification = classifyExpedientType(dummyPractices, diagField?.value || '');
            if (classification.confidence >= 60) {
                        setExpedientType(classification.suggestedType);
            }
        }
    ...
    // Handler: Skip IA → go directly to Step 2 (manual)
    const handleSkipToManual = useCallback(() => {
        const cosNum = generateCoseguroNumber();
                    ...
                    // Legacy handleAIParsed kept for backward compat — delegates to new flow
        ...
        const affiliateId = data.affiliate?.trim();
                    if (affiliateId) setAffSearch(affiliateId);
    ...
    const resetForm = () => {
                            ...
                            // Wizard reset
                            setWizardStep(1); setCoseguroNumber(''); setReviewData(null);
    };
    ...
    const totalValue = practiceItems.reduce((sum, pi) => sum + (pi.practice.financial_value || 0) * pi.quantity, 0);
                            ...
                            // ── Wizard helpers ──
                            const WIZARD_STEPS = [
                            {step: 1, label: 'Cargar Pedido', icon: Upload },
                            {step: 2, label: 'Completar Solicitud', icon: FileText },
                            {step: 3, label: 'Confirmar y Enviar', icon: ClipboardCheck },
                            ] as const;

    const currentExpTypeConfig = EXPEDIENT_TYPES.find(t => t.value === expedientType);
                            ...
                            <div className="max-w-3xl mx-auto p-4 space-y-5">
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    {coseguroNumber && (
                                        <div className="text-right">
                                            <span className="text-[10px] uppercase tracking-wider text-muted-foreground">Nro. Orden</span>
                                            <p className="text-sm font-mono font-bold text-blue-700">{coseguroNumber}</p>
                                        </div>
                                    )}
                                </div>

                                {/* ── Progress Steps ── */}
                                <div className="flex items-center gap-1 bg-muted/30 rounded-xl p-1.5">
                                    {WIZARD_STEPS.map(({ step, label, icon: Icon }, idx) => {
                                        const isActive = wizardStep === step;
                                        const isDone = wizardStep > step;
                                        return (
                                            <div key={step} className="flex items-center flex-1">
                                                <button
                                                    onClick={() => { if (isDone) setWizardStep(step as WizardStep); }}
                                                    disabled={!isDone}
                                                    className={`
                                    flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium w-full transition-all
                                    ${isActive
                                ...
                                `}
                                                ...
                                                {idx < WIZARD_STEPS.length - 1 && (
                                                    <ChevronRight className="h-3.5 w-3.5 mx-1 text-muted-foreground/40 flex-shrink-0" />
                                                )}
                                            </div>
                                        );
                                    })}
                                ...
                                {/* ══════════════════════════════════════ */}
                                {/* ═  STEP 2: FORMULARIO + CLASIFICACIÓN ═ */}
                                {/* ══════════════════════════════════════ */}
                                {wizardStep === 2 && (
                                    <>

                                        {/* Sticky header with order number and type */}
                                        {coseguroNumber && (
                                            <div className="sticky top-0 z-30 bg-background/95 backdrop-blur border-b rounded-lg px-4 py-2 flex items-center justify-between gap-3 -mx-4 mb-4 shadow-sm">
                                                <div className="flex items-center gap-3">
                                                    <span className="text-xs font-mono bg-blue-100 text-blue-700 px-2 py-0.5 rounded font-bold">
                                                        {coseguroNumber}
                                                    </span>
                                                    {currentExpTypeConfig && (
                                                        <span className={`text-xs px-2 py-0.5 rounded border ${currentExpTypeConfig.cls}`}>
                                                            {currentExpTypeConfig.label}
                                                        </span>
                                                    )}
                                                </div>
                                                {affiliate && (
                                        ...
                                        <div className="mb-6">
                                            ...
                                            <Tabs
                                                value={expedientType}
                                                onValueChange={(val) => {
                                                    setExpedientType(val as ExpedientType);
                                                    setRulesEvaluated(false);
                                                    setRulesResult(null);
                                                }}
                                                className="w-full"
                                            >
                                                <TabsList className="w-full flex-wrap h-auto bg-muted/50 p-1 rounded-xl justify-start">
                                                ...
                                                data-[state=active]:${t.cls} data-[state=active]:shadow-sm data-[state=active]:ring-1 data-[state=active]:ring-current/20
                                            `}
                                                            >
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            {/* ══════════════════════════════════════ */}
                                            {/* ═  ASISTENCIA IA (SUGERENCIAS)       ═ */}
                                            {/* ══════════════════════════════════════ */}
                                            {(aiPriorityResult || coherenceResult) && (
                                                <div className="border border-indigo-200 bg-indigo-50/50 dark:bg-indigo-950/20 dark:border-indigo-800 rounded-xl overflow-hidden mt-4">
                                                    <div className="bg-indigo-100/50 dark:bg-indigo-900/30 px-3 py-1.5 border-b border-indigo-200 dark:border-indigo-800 flex items-center gap-1.5">
                                                        ...
                                                        {coherenceResult && (
                                                            <div className="space-y-1.5">
                                                                ...
                                                                <p className="text-[11px] text-muted-foreground leading-relaxed">
                                                                    {coherenceResult.isCoherent
                                                                        ? 'El diagnóstico y las prácticas solicitadas son coherentes entre sí.'
                                                                        : coherenceResult.warning || 'Existen discrepancias entre el diagnóstico y las prácticas solicitadas.'}
                                                                </p>
                                                            </div>
                                                        )}
                                                        ...
                                                        {aiPriorityResult && (
                                                            <div className="space-y-1.5">
                                                                <div className="flex items-center gap-1.5">
                                                                    ...
                                                                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-wider ${aiPriorityResult.level === 'alta' ? 'bg-red-100 text-red-700 border-red-200' :
                                                                        aiPriorityResult.level === 'media' ? 'bg-amber-100 text-amber-700 border-amber-200' :
                                                                            'bg-green-100 text-green-700 border-green-200'
                                                                        } border`}>
                                                                        {aiPriorityResult.level}
                                                                    </span>
                                                                </div>
                                                                <p className="text-[11px] text-muted-foreground leading-relaxed">
                                                                    {aiPriorityResult.reason}
                                                                </p>
                                                            </div>
                                                        )}
                                                    </div>
            ...
            {/* ═  VALIDACIONES INLINE               ═ */}
                                            ...
                                            {/* ══════════════════════════════════════ */}
                                            <div className="pt-6 border-t mt-8 flex justify-between items-center bg-background/50 backdrop-blur sticky bottom-0 p-4 -mx-4 rounded-b-xl border-t shadow-[0_-4px_6px_-1px_rgb(0,0,0,0.05)]">
                                                <Button
                                                    variant="outline"
                                                    onClick={() => setWizardStep(1)}
                                                    className="px-8 h-12 text-sm font-medium border-2 hover:bg-slate-50 transition-all rounded-xl shadow-sm"
                                                >
                                                    <ChevronLeft className="h-4 w-4 mr-2" />
                                                    Volver al Pedido Médico
                                                </Button>

                                                <Button
                                                    onClick={() => setWizardStep(3)}
                                                    disabled={!canSubmit}
                                                    className="bg-primary hover:bg-primary/90 text-primary-foreground px-10 h-12 text-sm font-bold shadow-lg shadow-primary/20 transition-all hover:scale-[1.02] active:scale-[0.98] rounded-xl flex items-center gap-2 group"
                                                >
                                                    Revisar y Enviar
                                        ...
                                        {/* ══════════════════════════════════════ */}
                                        {wizardStep === 3 && (
                                            <div className="border-2 border-green-200 rounded-2xl bg-green-50/30 p-6 space-y-4">
                                                <div className="flex items-center gap-2">
                                                    <ClipboardCheck className="h-5 w-5 text-green-600" />
                                                    <h3 className="text-lg font-bold text-green-800">Resumen antes de enviar</h3>
                                                </div>
                                                ...
                                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                                                    ...
                                                    {coseguroNumber && (
                                                        <div className="bg-white rounded-lg border p-3">
                                                            <span className="text-[10px] uppercase tracking-wider text-muted-foreground">Nro. Orden</span>
                                                            <p className="font-mono font-bold text-blue-700">{coseguroNumber}</p>
                                                        </div>
                                                    )}
                                                    {affiliate && (
                                                        <div className="bg-white rounded-lg border p-3">
                                                            <span className="text-[10px] uppercase tracking-wider text-muted-foreground">Afiliado</span>
                                                            <p className="font-medium">{affiliate.full_name}</p>
                                                            <p className="text-xs text-muted-foreground">{affiliate.document_number}</p>
                                                        </div>
                                                    )}
                                                    {currentExpTypeConfig && (
                                                        <div className="bg-white rounded-lg border p-3">
                                                            <span className="text-[10px] uppercase tracking-wider text-muted-foreground">Tipo</span>
                                                            <p className={`font-medium ${currentExpTypeConfig.cls} inline-block px-2 py-0.5 rounded`}>
                                                                {currentExpTypeConfig.label}
                                                            </p>
                                                        </div>
                                                    )}
                                                    {(diagnosis || diagnosisCode) && (
                                                        <div className="bg-white rounded-lg border p-3">
                                                            <span className="text-[10px] uppercase tracking-wider text-muted-foreground">Diagnóstico</span>
                                                            <p className="font-medium">{diagnosis || diagnosisCode}</p>
                                                        </div>
                                                    )}
                                                    {doctorName && (
                                                        <div className="bg-white rounded-lg border p-3">
                                                            <span className="text-[10px] uppercase tracking-wider text-muted-foreground">Médico</span>
                                                            <p className="font-medium">{doctorName}</p>
                                                            {doctorRegistration && <p className="text-xs text-muted-foreground">Mat.: {doctorRegistration}</p>}
                                                        </div>
                                                    )}
                                                    {prescriptionDate && (
                                                        <div className="bg-white rounded-lg border p-3">
                                                            <span className="text-[10px] uppercase tracking-wider text-muted-foreground">Fecha Prescripción</span>
                                                            <p className="font-medium">{prescriptionDate}</p>
                                                        </div>
                                                    )}
                                                </div>
                                                ...
                                                {practiceItems.length > 0 && (
                                                    <div className="bg-white rounded-lg border p-3">
                                                        <span className="text-[10px] uppercase tracking-wider text-muted-foreground">Prácticas ({practiceItems.length})</span>
                                                        <div className="mt-1.5 space-y-1">
                                                            {practiceItems.map((pi, i) => {
                                                                const rColor = pi.ruleResult?.result
                                                                    ? RULE_COLORS[pi.ruleResult.result]
                                                                    : null;
                                                                return (
                                                                    <div key={i} className="flex items-center gap-2 text-sm">
                                                                        {rColor && (
                                                                            <span className={`inline-block w-2 h-2 rounded-full ${rColor.bg} border ${rColor.border}`} />
                                                                        )}
                                                                        <span className="font-mono text-xs text-muted-foreground">{pi.practice.code}</span>
                                                                        <span className="flex-1 truncate">{pi.practice.description}</span>
                                                ...
                                                {files.length > 0 && (
                                                    <div className="bg-white rounded-lg border p-3">
                                                        <span className="text-[10px] uppercase tracking-wider text-muted-foreground">Adjuntos ({files.length})</span>
                                                        <div className="mt-1 space-y-0.5">
                                                            {files.map((f, i) => (
                                                                <p key={i} className="text-xs text-muted-foreground">📎 {f.file.name}</p>
                                                            ))}
                                                        </div>
                                                    </div>
                                                )}
                                                ...
                                                {auditorsList.length > 0 && (
                                                    <div className="bg-white rounded-lg border p-3">
                                                        <span className="text-[10px] uppercase tracking-wider text-muted-foreground flex items-center gap-1.5 mb-2">
                                    ...
                                    `}
                                                ...
                                                <div className="flex items-center gap-3 pt-4 border-t">
                                                    ...
                                                    <Button
                                                        className="flex-1 bg-green-600 hover:bg-green-700 text-white font-semibold py-6 rounded-xl text-base"
                                                        disabled={submitting}
                                                    ...
                                                    >
                                                        {submitting ? (
                                                            ...
                                                            <><Send className="h-4 w-4 mr-2" /> Confirmar y Enviar Solicitud</>
                                                        )}
                                                    </Button>
                                                </div>
                                            </div>
